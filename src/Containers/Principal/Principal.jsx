import React from 'react'
import Facebook from '../../../img/logo-f.jpg'
import Logo from '../../../img/saskibaloi ermua.png'
import Centro from '../../../img/centro.jpg'
import Inscripciones from '../../../img/inscripciones.jpeg'
import Loteria from '../../../img/loteria.jpeg'
import Camisetas from '../../../img/camisetas.jpeg'
import Inscripciones2024 from '../../../img/inscripciones2024.jpeg'

const FacebookSVG = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 16 16">
    <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z"/>
  </svg>
);

const InstagramSVG = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="currentColor" class="bi bi-instagram" viewBox="0 0 16 16">
  <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599.28.28.453.546.598.92.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598 2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233 0-2.136.008-2.388.046-3.231.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92.28-.28.546-.453.92-.598.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334z"/>
</svg>
);
const TwitterSVG = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="39" height="39" fill="currentColor" class="bi bi-twitter" viewBox="0 0 16 16">
  <path d="M5.026 15c6.038 0 9.341-5.003 9.341-9.334 0-.14 0-.282-.006-.422A6.685 6.685 0 0 0 16 3.542a6.658 6.658 0 0 1-1.889.518 3.301 3.301 0 0 0 1.447-1.817 6.533 6.533 0 0 1-2.087.793A3.286 3.286 0 0 0 7.875 6.03a9.325 9.325 0 0 1-6.767-3.429 3.289 3.289 0 0 0 1.018 4.382A3.323 3.323 0 0 1 .64 6.575v.045a3.288 3.288 0 0 0 2.632 3.218 3.203 3.203 0 0 1-.865.115 3.23 3.23 0 0 1-.614-.057 3.283 3.283 0 0 0 3.067 2.277A6.588 6.588 0 0 1 .78 13.58a6.32 6.32 0 0 1-.78-.045A9.344 9.344 0 0 0 5.026 15z"/>
</svg>
);

const TiktokSVG = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-tiktok" viewBox="0 0 16 16">
  <path d="M9 0h1.98c.144.715.54 1.617 1.235 2.512C12.895 3.389 13.797 4 15 4v2c-1.753 0-3.07-.814-4-1.829V11a5 5 0 1 1-5-5v2a3 3 0 1 0 3 3V0Z"/>
</svg>
);

const YoutubeSVG = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-youtube" viewBox="0 0 16 16">
  <path d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.007 2.007 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.007 2.007 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31.4 31.4 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.007 2.007 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A99.788 99.788 0 0 1 7.858 2h.193zM6.4 5.209v4.818l4.157-2.408L6.4 5.209z"/>
</svg>
);

const Siguenos = ({ Imagen, Nombre, Link, Subnombre}) => {
  
  return (
    <article className='tw-followCard'>
      <header className='tw-followCard-header'>
      {Imagen}
        <div className='tw-followCard-info'>
          <strong>{Nombre}</strong>
          <span className='tw-followCard-infoUsername'></span>
          <span>{Subnombre}</span>
        </div>
      </header>

      <aside>
      <button className='tw-followCard-button' onClick={() => window.location.href = Link}>
    Seguir
    </button>

        
      </aside>
      <br />
    </article>
  );
};


const Futuro = ({ Noticia, Descripcion, Fecha, Imagen }) => {
    return (
      <article className='tw-followCard1'>
        <header className='tw-followCard-header'>
          <div className='tw-followCard-info'>
            <strong>{Noticia}</strong>
            <span className='tw-followCard-infoUsername'>{Descripcion}</span>
            <img src={Imagen} className='imagen-noticia' />
            <span>{Fecha}</span>
          </div>
        </header>
      </article>
    );
  };
  



const Clasificacion = () => {
    return (
    <table class="table">
  <thead>
    <tr>
      <th scope="col">#</th>
      <th scope="col">First</th>
      <th scope="col">Last</th>
      <th scope="col">Handle</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <th scope="row">1</th>
      <td>Mark</td>
      <td>Otto</td>
      <td>@mdo</td>
    </tr>
    <tr>
      <th scope="row">2</th>
      <td>Jacob</td>
      <td>Thornton</td>
      <td>@fat</td>
    </tr>
    <tr>
      <th scope="row">3</th>
      <td colspan="2">Larry the Bird</td>
      <td>@twitter</td>
    </tr>
  </tbody>
</table>
    )
}

const Partidos = ({ Grupo, Resultado, Rival, Estado, Fecha }) => {
    let estadoClass = '';
  
    if (Estado === 'Victoria') {
      estadoClass = 'estado-victoria';
    } else if (Estado === 'Derrota') {
      estadoClass = 'estado-derrota';
    } else if (Estado === 'Empate') {
      estadoClass = 'estado-empate';
    }
  
    return (
      <article className='tw-followCard'>
        <header className='tw-followCard-header'>
          <img src={Logo} className='tw-followCard-avatar' />
          <div className='tw-followCard-info'>
            <strong>Ermua {Grupo} vs {Rival}</strong>
            <span className={`tw-followCard-infoUsername `}>{Resultado}</span>
            <span>{Fecha}</span>
          </div>
        </header>
  
        <aside>
          <button className={`tw-followCard-button ${estadoClass}`}>{Estado}</button>
        </aside>
      </article>
    );
  };
  

const Noticias = () => {
    return (
        <h1 style={{ fontSize: '2em' }}>Noticias del equipo</h1>
    )
}

const Principal = () => {
  return (
    <div className="flex-container">
    <div className="flex-container-izquierda">
      <div className="flex-item">
        <h1 style={{ fontSize: '2em' }}>Siguenos</h1>
        <Siguenos  Imagen={<FacebookSVG />}  Nombre="Ermua Saskibaloi Eskola" Link="https://www.facebook.com/saskiermua/?show_switched_toast=0&show_invite_to_follow=0&show_switched_tooltip=0&show_podcast_settings=0&show_community_review_changes=0&show_community_rollback=0&show_follower_visibility_disclosure=0"/>
        <Siguenos Imagen={<InstagramSVG />} Nombre="saskiermua" Subnombre="Ermua Saskibaloi Ermua" Link="https://www.instagram.com/saskiermua/?igshid=MzRlODBiNWFlZA%3D%3D"/>
        <Siguenos Imagen={<TwitterSVG/>}  Nombre="Saskibaloi Eskola Ermua" Subnombre="@SaskiErmua" Link="https://twitter.com/SaskiErmua?t=s0xWX-kfovDGi0bvUJ2NiQ&s=09" />
        <Siguenos Imagen={<TiktokSVG/>}  Nombre="ermuasaskibaloies" Subnombre="Saski Ermua" Link="https://www.tiktok.com/@ermuasaskibaloies?_t=8dFUrMjJtxz&_r=1"/>
        <Siguenos Imagen={<YoutubeSVG/>} Nombre=" Saskibaloi Eskola Ermua " Subnombre="@saskibaloieskolaermua5498" Link="https://www.youtube.com/@saskibaloieskolaermua5498" />
      </div>
    </div>
    <div className="flex-container-centro">
      <div className="flex-item">
        <img src={Centro} className="imagen-centro"/>
        <h2 className="titulo-centro">Club Saskibaloi Ermua</h2>
        <h4 className="subtitulo-centro"><em>"Los sueños se hacen realidad"</em></h4>
        <p className="historia">Este proyecto se empezó por el 2020 pero por el covid no se pudo arrancar hasta septiembre 2021.
          Intentar sacar un grupo de baloncesto escolar en un sitio como Ermua con tradición niños fútbol y niñas balonmano y donde apenas habido baloncesto escolar es muy difícil. Muchas horas de reuniones, llamadas… Decisiones difíciles y siempre pensando en los niñ@s.
          Momentos malos de tirar la toalla ,
          dejar el equipo de Ermua Basket, crear un nuevo club….
          Cada día un problema resuelto y 3 nuevos más…
          Esté fin de semana empiezan los 4 equipos, con ilusión y ganas de baloncesto.
          Por el baloncesto en Ermua.
          Que decir que sin la ayuda de Ina Mendibe que siempre esta dispuesto para lo que nos haga falta.
          La directiva de este equipo la conforman Mónica, Luis, Fernan, Juan Carlos , Jaime sin ellos esto no funcionaria.
          Gracias a Rubén por toda tu colaboración.
          Feredu construcciones y reformas por patrocinarnos y Ayuntamiento y Federación bizkaina baloncesto por su colaboración.
          </p>
      </div>
    </div>
    <div className="flex-container-derecha">
      <div className="flex-item">
        <h1 style={{ fontSize: '2em' }}>Proximos encuentros</h1>
        <Partidos Grupo="premini" Rival="Berango"  Fecha="14-2-2023" Estado="Victoria"/>
        <Partidos Grupo="mini" Rival="Basauri"  Fecha="14-2-2023" Estado="Empate"/>
        <Partidos Grupo="infantil" Rival="Artxandape"  Fecha="14-2-2023" Estado="Derrota"/>
        <Partidos Grupo="infantil mixto" Rival="Durango"  Fecha="14-2-2023" Estado="Proximamente..."/>
        <Noticias />
        <Futuro Noticia="Inscripciones abiertas" Descripcion="Para enviar el formulario de inscripcion al club vaya arriba de la pagina y pulse al boton, a continuacion le saltara una ventana con el formulario" Imagen={Inscripciones2024} Fecha="2024/2025"/>
        <Futuro Noticia="Loteria de navidad" Descripcion={
    <>
      Participaciones 5€,<br />
      Juegas 4€ y 1€ donativo para el club
    </>
  } Imagen={Loteria} Fecha="22/12/2023"/>
  <Futuro Noticia="Tenemos camisetas del club" Descripcion="Tallas S M L XL 2XL 3XL" Imagen={Camisetas}  />
  


      </div>
    </div>
  </div>

  )
}

export default Principal
